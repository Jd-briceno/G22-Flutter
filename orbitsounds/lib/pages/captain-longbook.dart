...
class _LongbookState extends State<Longbook> {
  ...
  // === Canciones ===
  List<String> _sessionAlbumArts = []; // 🆕 Portadas de la última sesión
  ...
  void initState() {
    ...
    _loadLatestSessionTracks(); // 🔥 cargar carátulas reales si existen
  }
    ...
    try {
      ...
      await analytics.logEvent(
        ...
        parameters: {
          ...
          'from_reminder': NotificationService.cameFromReminder ? 1 : 0, // usa 1 o 0
        },
      );

      // 🔁 Una vez logueado el evento, reseteamos la bandera
      NotificationService.cameFromReminder = false;
  ...
  Future<void> _loadLatestSessionNotes() async {
    final user = FirebaseAuth.instance.currentUser;
    if (user == null) {
      debugPrint("⚠️ No hay usuario logueado para cargar notas.");
      return;
    }
    ...
    try {
      ...
      if (sessionSnapshot.docs.isEmpty) {
        debugPrint("⚠️ No hay sesiones para cargar notas.");
        return;
      }

      final latestDoc = sessionSnapshot.docs.first;
      final data = latestDoc.data();

      final existingNotes = data["notes"]?.toString().trim() ?? "";
      ...
      if (existingNotes.isNotEmpty) {
        debugPrint("📝 Notas cargadas desde Firestore: '${existingNotes.substring(0, min(50, existingNotes.length))}...'");
      } else {
        debugPrint("ℹ️ Última sesión sin notas, campo vacío.");
      }
    } catch (e, st) {
      debugPrint("❌ Error cargando notas de la última sesión: $e\n$st");
    }
  }

  Future<void> _saveNotesToSession() async {
    final user = FirebaseAuth.instance.currentUser;
    if (user == null) {
    ...
    try {
      ...
      if (sessionSnapshot.docs.isEmpty) {
        debugPrint("⚠️ No hay sesiones existentes. Creando una nueva...");
        ...
        debugPrint("✅ Nueva sesión creada con notas");
        return;
      }

      // 2️⃣ Actualizar la sesión más reciente
      final latestDoc = sessionSnapshot.docs.first.reference;
      await latestDoc.update({
        "notes": _notesController.text.trim(),
        "notes_updated_at": FieldValue.serverTimestamp(),
      });

      debugPrint("✅ Notas guardadas en la sesión más reciente.");
    } catch (e) {
      debugPrint("❌ Error guardando notas: $e");
    }
  }

  // 🎵 Cargar carátulas (albumArt) de los tracks de la última sesión
  Future<void> _loadLatestSessionTracks() async {
    final user = FirebaseAuth.instance.currentUser;
    if (user == null) return;
    ...
    try {
      ...
      if (sessionSnapshot.docs.isEmpty) {
        debugPrint("⚠️ No hay sesiones para cargar tracks.");
        return;
      }

      final latestSession = sessionSnapshot.docs.first;
      final sessionId = latestSession.id;
      final data = latestSession.data();

      // 🔍 Obtener el array "tracks" dentro del documento
      final List<dynamic>? tracksArray = data["tracks"];

      if (tracksArray == null || tracksArray.isEmpty) {
        debugPrint("⚠️ La sesión $sessionId no tiene tracks guardados.");
        return;
      }

      // Extraer las URLs de albumArt
      final albumArts = tracksArray
          .map((item) => (item as Map<String, dynamic>)["albumArt"])
          .whereType<String>() // ✅ Filtra solo strings no nulos
          .where((url) => url.isNotEmpty)
          .toList();

      setState(() {
        _sessionAlbumArts = albumArts;
      });

      debugPrint("🎨 Portadas cargadas desde sesión $sessionId: $_sessionAlbumArts");
    } catch (e) {
...
<path d="M191 466H6.00001" stroke="#E9E8EE" stroke-width="2"/>
                      ...
                      // === BOTÓN DE MEDITACIÓN TONAL (NUEVO) ===
                      Positioned(
                        left: 255,
                        top: 30,
                        child: IconButton(
                          ...
                          onPressed: () async {
                            // 1️⃣ Normalizar emociones correctamente
                            final normalized = _selectedEmotions
                                .map((e) => e.trim().toUpperCase())
                                .toList();

                            print("🎯 Normalizadas (para fallback): $normalized");

                            // 2️⃣ Tomar la emoción principal o UNKNOWN
                            final emotion = normalized.isNotEmpty ? normalized.first : "UNKNOWN";

                            print("🎨 Emoción usada para fallback: $emotion");

                            // 3️⃣ Obtener mensaje
                            final zenMessage = sumiZenDefaults[emotion] ?? sumiZenDefaults["UNKNOWN"]!;

                            print("📝 Mensaje seleccionado: $zenMessage");

                            // 4️⃣ BPM
                            final bpm = _latestTrackBPM ?? 80;
                            ...
                            showDialog(
                              ...
                              builder: (_) => TonalBreathingModal(
                                bpm: bpm,
                                emotions: normalized,
                                zenMessage: zenMessage,
                              ),
                            );
                      ...
                      Positioned(
                        ...
                        child: IconButton(
                          ...
                          onPressed: () async {
                            if (_isEditing) {
                              await _saveNotesToSession(); // 🔹 Guarda cuando salimos del modo edición
                            }
                            setState(() {
                      ...
                      child: Container(
                        ...
                        child: SingleChildScrollView(
                          ...
                          child: Row(
                            ...
                            children: (_sessionAlbumArts.isNotEmpty
                                    ? _sessionAlbumArts
                                    : _albumCovers)
                                .map((art) {
                              final isNetwork = art.startsWith("http");
                              return Padding(
